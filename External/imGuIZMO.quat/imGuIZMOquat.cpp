...
{                          
	vec3 l = vec3::make((light<.5) ? .5f : light); 
    vec3 a = vec3::make(atten>.25  ? .25f : atten);

	vec3 c(
		((color.asVec3() + l*.5f) * l) *.75f
		+ (a*color.asVec3()*.45f)
		+ (a*.25f)
		);
...
{                        
	vec4 c = {
		float(color & 0xff)/255.f,float((color>>8) & 0xff)/255.f,float((color>>16) & 0xff)/255.f, 1.0f
	};
...
inline vec3 &adjustPlane(vec3 &coord)
{
	...
	coord *= vec3::make(1.0f, 2.0f, 2.0f);
    return coord;
}
...
inline vec3 &adjustDir(vec3 &coord)
{
    ...
    coord *= vec3::make(1.0f, 3.0f, 3.0f);
    return coord;
}
...
{
    return ((axis == imguiGizmo::axisIsY) ? vec3::make(-v.y, v.x, v.z) : // rotation Z 90'
           ((axis == imguiGizmo::axisIsZ) ? vec3::make(-v.z, v.y, v.x) : // rotation Y 90'
                                          v));
}
////////////////////////////////////////////////////////////////////////////
//
...
bool imguiGizmo::drawFunc(const char* label, float size)
{
    ...
    vec3 resizeAxes
		= ((drawMode&modeDual) && (axesResizeFactor.x>.75f))
		? vec3::make(.75f,axesResizeFactor.y, axesResizeFactor.z)
		: axesResizeFactor
		;
	...
	struct Util {
		static void getTrackball(
			quat &q, vg::vImGuIZMO &track, const ImVec2& controlPos, const ImVec2& innerSize
			, imguiGizmo & gizmo
			, ImGuiIO& io
			, const vgModifiers vgMods
			, bool & value_changed
			)
		{
		...
		#ifndef IMGUIZMO_USE_ONLY_ROT
			if(gizmo.drawMode&modePanDolly || io.MouseWheel!=0) {
				float screenFactor = 1.f/(io.DisplaySize.x<io.DisplaySize.y ? io.DisplaySize.x : io.DisplaySize.y);
				track.setPosition(gizmo.posPanDolly);
				track.setDollyControl(gizmo.buttonPanDolly, dollyMod);
				track.setPanControl(gizmo.buttonPanDolly, panMod);
				...
				if((!track.isDollyActive() && !track.isPanActive() && io.MouseWheel==0)) {
					q = track.getRotation();
				}
				else {
					gizmo.posPanDolly = track.getPosition();
				}
			} else {
    ...
    if (ImGui::IsItemActive()) {
		...
		if(ImGui::IsMouseDragging(0)) {
			Util::getTrackball(qtV, track, controlPos, innerSize, *this, io, vgMods, value_changed);
		}
		if((drawMode&modeDual) && ImGui::IsMouseDragging(1)) {
			Util::getTrackball(qtV2, track, controlPos, innerSize, *this, io, vgMods, value_changed); // if dual mode... move together
		}
		...
		if(highlighted && (drawMode&modeDual) && ImGui::IsMouseDragging(1)) {
			...
			Util::getTrackball(qtV, track, controlPos, innerSize, *this, io, vgMods, value_changed);
			Util::getTrackball(qtV2, track, controlPos, innerSize, *this, io, vgMods, value_changed);
		}
#ifndef IMGUIZMO_USE_ONLY_ROT
	...
	struct Utils {
			...
			) 
		{
			...
			const int* itTess = sphereTess.begin();
			for(vec3* itVtx = sphereVtx.begin(); itVtx != sphereVtx.end(); )  {
				for(int h=0; h<3; h++, itTess++) {
					vec3 coord = _q  * (*itVtx++ * solidResizeFactor);        //Rotate

					uv[h] = normalizeToControlSize(coord.x,coord.y, controlPos, halfSquareSize);
				...
				addTriangle(uv, col, wpUV, draw_list);
			}
		};
			...
			)
		{
			...
			for(vec3* itNorm = cubeNorm.begin(), *itVtx  = cubeVtx.begin() ; itNorm != cubeNorm.end();) {
				...
				for(int i = 0; i<4; ) {
					...
					uv[i++] = normalizeToControlSize(coord.x,coord.y, controlPos, halfSquareSize);
				}                    
				addQuad(
					addLightEffect(vec4::make(abs(*itNorm++),1.0f), norm.z, coord.z)
					, uv, wpUV, draw_list
					);
			}
		};
			...
			)  
		{
			...
			vec3* itNorm = planeNorm.begin();
			vec3* itVtx  = planeVtx.begin();
			for( ; itNorm != planeNorm.end();) {
				vec3 coord;
				vec3 norm = _q * *itNorm;
				for(int i = 0; i<4; ) {
					coord = _q  * (*itVtx++ * solidResizeFactor);
				...
				itNorm++;
				addQuad(
					addLightEffect(vec4::make(planeColor.x, planeColor.y, planeColor.z, planeColor.w), norm.z, coord.z)
					, uv
					, wpUV
					, draw_list
					);
			...
			for(int n = 0; n < 4; n++) { //Arrow: 2 Cone -> (Surface + cap) + 2 Cyl -> (Surface + cap)
				for(int arrowAxis = 0; arrowAxis < 3; arrowAxis++) { // draw 3 axes
					...
					if((side == backSide  && arrowCoordZ > 0) || (side == frontSide && arrowCoordZ <= 0)) {
						if (!gizmo.showFullAxes && (i == CYL_CAP)) continue; // skip if cylCap is hidden
						if (i <= CONE_CAP) continue;  // do not draw cone
						else skipCone = false;
					}

					ImVector<vec3> *ptrVtx = arrowVtx + i;
					draw_list->PrimReserve(ptrVtx->size(), ptrVtx->size()); // // reserve vtx

					vec3* itVtx = ptrVtx->begin();
					vec3* itNorm = (arrowNorm+i)->begin();

					for(; itVtx != ptrVtx->end(); ) { //for all Vtx
						...
						// reposition starting point...
							...
							if((skipCone && coord.x <= 0) || 
							   (!gizmo.showFullAxes && (coord.x < arrowStartingPoint)) ) coord.x =  arrowStartingPoint;
						//transform
							...
							col[h] = addLightEffect(
								vec4::make(float(arrowAxis==axisIsX),float(arrowAxis==axisIsY),float(arrowAxis==axisIsZ), 1.0)
								, norm.z, coord.z
								);
						}
						addTriangle( uv, col, wpUV, draw_list );
		...
		static void drawComponent(
			const int idx, const quat &q, ptrFunc func
			, ImDrawList* draw_list
			, const vec3& resizeAxes
			, ImVec2 uv[4]
			, ImU32 col[4]
		...
		{
			ImVector<vec3> *ptrVtx = arrowVtx+idx;
			draw_list->PrimReserve(ptrVtx->size(), ptrVtx->size()); // reserve vtx

			vec3* itVtx = ptrVtx->begin();
			vec3* itNorm = (arrowNorm+idx)->begin();

			for(; itVtx != ptrVtx->end(); ) { 
	#if !defined(imguiGizmo_INTERPOLATE_NORMALS)
				vec3 norm = (_q * *itNorm++);
	#endif
				for(int h=0; h<3; h++) {
					...
					col[h] = addLightEffect(
						vec4::make(directionColor.x, directionColor.y, directionColor.z, 1.0), norm.z, coord.z>0 ? coord.z : coord.z*.5
						);
				}
				addTriangle(uv, col, wpUV, draw_list);
			}
		...
		//////////////////////////////////////////////////////////////////
		static void dirArrow(
			const quat &q, int mode
			, const quat& _q
		...
		{
			vec3 arrowCoord(_q * vec3::make(1.0f, 0.0f, 0.0f));

			ptrFunc func = (mode & modeDirPlane) ? adjustPlane : adjustDir;

			if(arrowCoord.z <= 0) {
				for(int i = 0; i <  4; i++) drawComponent(
					i, q, func
					, draw_list, resizeAxes, uv, col, wpUV, controlPos, halfSquareSize
					);
				if(mode & modeDirPlane) drawPlane(draw_list, _q, uv, wpUV, controlPos, halfSquareSize);
			}
			else                  {
				if(mode & modeDirPlane) drawPlane();
				for(int i = 3; i >= 0; i--) drawComponent(
					i, q, func
					, draw_list, resizeAxes, uv, col, wpUV, controlPos, halfSquareSize
					);
			}
		...
		//////////////////////////////////////////////////////////////////
		static void spotArrow(const quat &q, const float arrowCoordZ) 
		{
			if(arrowCoordZ > 0) { 
	...
	#define CENTER_HELPER_Y -.85f
		//////////////////////////////////////////////////////////////////
		static void drawRotationHelper() {
	...
	if(drawMode & (modeDirection | modeDirPlane)) {
        ...
        if(drawMode & modeDual) {
			vec3 spot(qtV2 * vec3::make(-1.0f, 0.0f, .0f)); // versus opposite
            if(spot.z>0) { Utils::draw3DSystem(); Utils::spotArrow(normalize(qtV2),spot.z); }
            else         { Utils::spotArrow(normalize(qtV2),spot.z); Utils::draw3DSystem(); }
        } else Utils::draw3DSystem();
    }
    ...
    if(vgModsActive && (ImGui::IsItemHovered() && (!ImGui::IsMouseDown(0) && !ImGui::IsMouseDown(1)) )) {
        ...
        if(drawMode & modePanDolly) {
            if(panMod & vgMods)        Utils::drawPanHelper();
            else if(dollyMod & vgMods) Utils::drawDollyHelper();
        } else {
            Utils::drawRotationHelper();
        }
#else
        Utils::drawRotationHelper();
#endif
...
#   define V(x,y,z) sphereVtx.push_back(vec3::make(x, y, z))
