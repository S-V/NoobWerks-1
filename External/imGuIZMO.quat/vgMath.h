...
public:
    ...
    Vec2() {}
	Vec2(const VEC2_T& o) : x(o.x), y(o.y) {}
...
public:
	...
	//Vec3() {}
	//Vec3(const VEC3_T& other) : x(other.x), y(other.y), z(other.z) {}
	...
	static Vec3 make(T x, T y, T z) {
		Vec3 v = { x, y, z };
		return v;
	}
	static Vec3 make(T s) {
		Vec3 v = { s, s, s };
		return v;
	}
    ...
    Vec3 operator-() const { return Vec3::make(-x, -y, -z); }
	...
	Vec3 operator+(const Vec3& v) const { return Vec3::make( x + v.x, y + v.y, z + v.z ); }
    Vec3 operator-(const Vec3& v) const { return Vec3::make( x - v.x, y - v.y, z - v.z ); }
    Vec3 operator*(const Vec3& v) const { return Vec3::make( x * v.x, y * v.y, z * v.z ); }
    Vec3 operator/(const Vec3& v) const { return Vec3::make( x / v.x, y / v.y, z / v.z ); }
    Vec3 operator*(T s)           const { return Vec3::make( x * s  , y * s  , z * s   ); }
    Vec3 operator/(T s)           const { return Vec3::make( x / s  , y / s  , z / s   ); }
...
public:
	...
	//Vec4() {};
 //   Vec4(const VEC4_T& other) : x(other.x), y(other.y), z(other.z), w(other.w)   {}
 ...
 //   explicit Vec4(T s, const VEC3_T& v) : x(s),   y(v.x), z(v.y), w(v.z) {}
	...
	static Vec4 make(T x, T y, T z, T w) {
		Vec4 result = { x, y, z, w };
		return result;
	}
	static Vec4 make(const VEC3_T& v, T w) {
	...
	Vec4 operator-() const { return Vec4::make(x, -y, -z, -w); }
    ...
    Vec4 operator+(const Vec4& v) const { return Vec4::make( x + v.x, y + v.y, z + v.z, w + v.w ); }
    Vec4 operator-(const Vec4& v) const { return Vec4::make( x - v.x, y - v.y, z - v.z, w - v.w ); }
    Vec4 operator*(const Vec4& v) const { return Vec4::make( x * v.x, y * v.y, z * v.z, w * v.w ); }
    Vec4 operator/(const Vec4& v) const { return Vec4::make( x / v.x, y / v.y, z / v.z, w / v.w ); }
    Vec4 operator*(T s)           const { return Vec4::make( x * s  , y * s  , z * s  , w * s   ); }
    Vec4 operator/(T s)           const { return Vec4::make( x / s  , y / s  , z / s  , w / s   ); }
...
public:
	...
	Quat() {};
    Quat(const QUAT_T& other)  : x(other.x), y(other.y), z(other.z), w(other.w)   {}
...
public:
 ...
 //   Mat3(const MAT3_T &)    = default;
	explicit Mat3(T s) {
		v[0] = VEC3_T::make(s, 0, 0);
		v[1] = VEC3_T::make(0, s, 0);
		v[2] = VEC3_T::make(0, 0, s);
	}
	...
	Mat3(T v0x, T v0y, T v0z,
         ...
         T v2x, T v2y, T v2z)
	{
		v[0] = VEC3_T::make(v0x, v0y, v0z);
		v[1] = VEC3_T::make(v1x, v1y, v1z);
		v[2] = VEC3_T::make(v2x, v2y, v2z);
	}
    ...
    Mat3 operator+(const Mat3& m) const { return Mat3( v[0] + m.v[0], v[1] + m.v[1], v[2] + m.v[2] ); }
    Mat3 operator-(const Mat3& m) const { return Mat3( v[0] - m.v[0], v[1] - m.v[1], v[2] - m.v[2] ); }
#define M(X,Y) (m##X * m.m##Y)
    Mat3 operator*(const Mat3& m) const
	{
		Mat3 result;
		result.m00 = M(00,00) + M(10,01) + M(20,02);
		result.m01 = M(01,00) + M(11,01) + M(21,02);
		result.m02 = M(02,00) + M(12,01) + M(22,02);

		result.m10 = M(00,10) + M(10,11) + M(20,12);
		result.m11 = M(01,10) + M(11,11) + M(21,12);
		result.m12 = M(02,10) + M(12,11) + M(22,12);

		result.m20 = M(00,20) + M(10,21) + M(20,22);
		result.m21 = M(01,20) + M(11,21) + M(21,22);
		result.m22 = M(02,20) + M(12,21) + M(22,22);
		return result;
	}
#undef M
    Mat3 operator*(T s) const { return Mat3( v[0] * s, v[1] * s, v[2] * s ); }
    Mat3 operator/(T s) const { return Mat3( v[0] / s, v[1] / s, v[2] / s ); }

	VEC3_T operator*(const VEC3_T& v) const { return VEC3_T::make(
...
public:
    ...
    explicit Mat4(T s)
	{
		v[0] = VEC4_T::make(s, 0, 0, 0);
		v[1] = VEC4_T::make(0, s, 0, 0);
		v[2] = VEC4_T::make(0, 0, s, 0);
		v[3] = VEC4_T::make(0, 0, 0, s);
	}
    ...
    explicit Mat4(const MAT3_T& m)
	{
		v[0] = VEC4_T::make(m.v[0],0);
		v[1] = VEC4_T::make(m.v[1],0);
		v[2] = VEC4_T::make(m.v[2],0);
		v[3] = VEC4_T::make(0, 0, 0, 1);
	}
    Mat4(T v0x, T v0y, T v0z, T v0w,
         T v1x, T v1y, T v1z, T v1w,
         T v2x, T v2y, T v2z, T v2w,
         T v3x, T v3y, T v3z, T v3w)
	{
		v[0] = VEC4_T::make(v0x, v0y, v0z, v0w);
		v[1] = VEC4_T::make(v1x, v1y, v1z, v1w);
		v[2] = VEC4_T::make(v2x, v2y, v2z, v2w);
		v[3] = VEC4_T::make(v3x, v3y, v3z, v3w);
	}
    ...
    Mat4 operator+(const Mat4& m) const { return Mat4( v[0] + m.v[0], v[1] + m.v[1], v[2] + m.v[2], v[3] + m.v[3] ); }
    Mat4 operator-(const Mat4& m) const { return Mat4( v[0] - m.v[0], v[1] - m.v[1], v[2] - m.v[2], v[3] - m.v[3] ); }
    Mat4 operator*(T s)           const { return Mat4( v[0] * s     , v[1] * s     , v[2] * s     , v[3] * s      ); }
    Mat4 operator/(T s)           const { return Mat4( v[0] / s     , v[1] / s     , v[2] / s     , v[3] / s      ); }
#define M(X,Y) (m##X * m.m##Y)
	Mat4 operator*(const Mat4& m) const
	{
		...
		result.m00 = M(00,00) + M(10,01) + M(20,02) + M(30,03);
		result.m01 = M(01,00) + M(11,01) + M(21,02) + M(31,03);
		result.m02 = M(02,00) + M(12,01) + M(22,02) + M(32,03);
		result.m03 = M(03,00) + M(13,01) + M(23,02) + M(33,03);
		
		result.m10 = M(00,10) + M(10,11) + M(20,12) + M(30,13);
		result.m11 = M(01,10) + M(11,11) + M(21,12) + M(31,13);
		result.m12 = M(02,10) + M(12,11) + M(22,12) + M(32,13);
		result.m13 = M(03,10) + M(13,11) + M(23,12) + M(33,13);

		result.m20 = M(00,20) + M(10,21) + M(20,22) + M(30,23);
		result.m21 = M(01,20) + M(11,21) + M(21,22) + M(31,23);
		result.m22 = M(02,20) + M(12,21) + M(22,22) + M(32,23);
		result.m23 = M(03,20) + M(13,21) + M(23,22) + M(33,23);

		result.m30 = M(00,30) + M(10,31) + M(20,32) + M(30,33);
		result.m31 = M(01,30) + M(11,31) + M(21,32) + M(31,33);
		result.m32 = M(02,30) + M(12,31) + M(22,32) + M(32,33);
		result.m33 = M(03,30) + M(13,31) + M(23,32) + M(33,33);

		return result;
	}
#undef M
	VEC4_T operator*(const VEC4_T& v) const { return VEC4_T::make( m00 * v.x + m10 * v.y + m20 * v.z + m30 * v.w,
...
// cast / conversion
//////////////////////////
TEMPLATE_TYPENAME_T inline VEC2_T::Vec2(const VEC3_T& v) : x(v.x), y(v.y) {}
//TEMPLATE_TYPENAME_T inline VEC3_T::Vec3(const VEC4_T& v) : v.x, v.y, v.z ) {}
...
TEMPLATE_TYPENAME_T inline VEC3_T cross(const VEC3_T& u, const VEC3_T& v) { return VEC3_T::make(
	u.y * v.z - u.z * v.y,
	u.z * v.x - u.x * v.z,
	u.x * v.y - u.y * v.x
	); }
...
TEMPLATE_TYPENAME_T inline VEC2_T abs(const VEC2_T& v) { return VEC2_T( tAbs(v.x), tAbs(v.y) ); }
TEMPLATE_TYPENAME_T inline VEC3_T abs(const VEC3_T& v) { return VEC3_T::make( tAbs(v.x), tAbs(v.y), tAbs(v.z) ); }
TEMPLATE_TYPENAME_T inline VEC4_T abs(const VEC4_T& v) { return VEC4_T::make( tAbs(v.x), tAbs(v.y), tAbs(v.z), tAbs(v.w) ); }
TEMPLATE_TYPENAME_T inline QUAT_T abs(const QUAT_T& q) { return QUAT_T( tAbs(q.w), tAbs(q.x), tAbs(q.y), tAbs(q.z) ); }
// sign
//////////////////////////
...
TEMPLATE_TYPENAME_T inline VEC3_T pow(const VEC3_T& b, const VEC3_T& e) { return VEC3_T::make( ::pow(b.x,e.x), ::pow(b.y,e.y), ::pow(b.z,e.z) ); }
TEMPLATE_TYPENAME_T inline VEC4_T pow(const VEC4_T& b, const VEC4_T& e) { return VEC4_T::make( ::pow(b.x,e.x), ::pow(b.y,e.y), ::pow(b.z,e.z), ::pow(b.w,e.w) ); }
// value_ptr
//////////////////////////
...
#undef M
	...
	VEC4_T v0r0 = m.v[0] * VEC4_T::make(inv.m00, inv.m10, inv.m20, inv.m30);
...
TEMPLATE_TYPENAME_T inline VEC3_T operator/(const T s, const VEC3_T& v) {  return VEC3_T::make( s/v.x, s/v.y, s/v.z ); }
TEMPLATE_TYPENAME_T inline VEC4_T operator/(const T s, const VEC4_T& v) {  return VEC4_T::make( s/v.x, s/v.y, s/v.z, s/v.w ); }
...
TEMPLATE_TYPENAME_T inline VEC3_T axis(QUAT_T const& q) {
	const T t1 = T(1) - q.w * q.w; if(t1 <= T(0)) return VEC3_T::make(0, 0, 1);
    const T t2 = T(1) / sqrt(t1);  return VEC3_T::make(q.x * t2, q.y * t2, q.z * t2); }
// trigonometric
//////////////////////////
...
TEMPLATE_TYPENAME_T inline MAT4_T frustum(T l, T r, T b, T t, T n, T f)
{
	...
	//#endif
		...
		typedef vgm::Vec2 vec2;
		typedef vgm::Vec3 vec3;
		typedef vgm::Vec4 vec4;
		typedef vgm::Quat quat;
		typedef vgm::Mat3 mat3;
		typedef vgm::Mat4 mat4;
		typedef mat3 mat3x3;
		typedef mat4 mat4x4;

	#ifdef VGIZMO_USES_HLSL_TYPES
		typedef vgm::Vec2 float2;
		typedef vgm::Vec3 float3;
		typedef vgm::Vec4 float4;
		typedef vgm::Mat3f float3x3;
		typedef vgm::Mat4f float4x4;
	#endif




#endif // ! #ifdef VGM_USES_TEMPLATE
...
// Internal vGizmo USES ONLY
    ...
    typedef vgm::VEC2_PRECISION tVec2;
    typedef vgm::VEC3_PRECISION tVec3;
    typedef vgm::VEC4_PRECISION tVec4;
    typedef vgm::QUAT_PRECISION tQuat;
    typedef vgm::MAT3_PRECISION tMat3;
    typedef vgm::MAT4_PRECISION tMat4;
