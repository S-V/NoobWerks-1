...
struct imguiGizmo
{
	quat qtV ; // Quaternion value
	quat qtV2; // Quaternion value
	...
	imguiGizmo()
	{
		qtV  = quat(1.0f, vec3::make(0.0f)); // Quaternion value
		qtV2 = quat(1.0f, vec3::make(0.0f)); // Quaternion value
#ifndef IMGUIZMO_USE_ONLY_ROT
		posPanDolly = vec3::make(0.f);
		buttonPanDolly = vg::evLeftButton;
#endif


		// internals
		drawMode = mode3Axes;
		axesOriginType = cubeAtOrigin;
		showFullAxes = false;
	}
    ...
    static void buildPlane   (const float size, const float thickness = planeThickness) {
		buildPolygon(vec3::make(thickness,size,size), planeVtx, planeNorm);
    }
    static void buildCube    (const float size) {
        buildPolygon(vec3::make(size), cubeVtx, cubeNorm);
    }
...
//      evSuperModifier   -> Super
    ...
    bool getTransforms(quat& q, const char* label, vec3& dir, float size) {
        ...
        else if(len< FLT_EPSILON) { dir = vec3::make(1.f, 0.f, 0.f); len = 1.0; }

        q = angleAxis(acosf(dir.x/len), normalize(vec3::make(FLT_EPSILON, -dir.z, dir.y)));

        bool ret = drawFunc(label, size);
        if (ret) dir = (q * vec3::make(1.0f, 0.0f, 0.0f)) * len ; //return vector with original length
    ...
    bool getTransforms(quat& q, const char* label, vec4& axis_angle, float size) {
        q = angleAxis(axis_angle.w,axis_angle.asVec3()); //g.ConvertFromAxisAngle();
   
        bool ret = drawFunc(label, size);
		if (ret) axis_angle = vec4::make(vec3(axis(q)),angle(q));
