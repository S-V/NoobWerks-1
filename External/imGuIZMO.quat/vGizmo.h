...
public:
    ...
    {
		rotationVector = tVec3::make(T(1));
		qtV    = tQuat(T(1), T(0), T(0), T(0));
		qtStep = tQuat(T(1), T(0), T(0), T(0));
		qtIdle = tQuat(T(1), T(0), T(0), T(0));

		rotationCenter = tVec3::make(T(0));

		tbScale = T(1);    //base scale sensibility
		fpsRatio = T(1);   //auto adjust by FPS (call idle with current FPS)
		qIdleSpeedRatio = T(.33); //autoRotation factor to speedup/slowdown
    ...
    void viewportSize(T w, T h) { 
		...
		offset = tVec3::make(T(.5 * width), T(.5 * height), T(0));
    }
    ...
    virtual void mouse( vgButtons button, vgModifiers mod, bool pressed, T x, T y)
    {
        ...
        if((button == tbRotationButton) && pressed) {
            if      (xRotationModifier & mod) { tbActive = true; rotationVector = tVec3::make(T(1), T(0), T(0)); activateMouse(x,y); }
            else if (yRotationModifier & mod) { tbActive = true; rotationVector = tVec3::make(T(0), T(1), T(0)); activateMouse(x,y); }
            else if (zRotationModifier & mod) { tbActive = true; rotationVector = tVec3::make(T(0), T(0), T(1)); activateMouse(x,y); }
        } else if((button == tbRotationButton) && !pressed) { 
            deactivateMouse(); rotationVector = tVec3::make(T(1)); tbActive = false; 
        }
    ...
    void updateGizmo() 
    {
		...
		struct Foo {
			static void vecFromPos(tVec3 &v, const tVec3& offset, const T minVal) {
				v -= offset;
				v /= minVal;
		...
		Foo::vecFromPos(a, offset, minVal);
        Foo::vecFromPos(b, offset, minVal);
...
//////////////////////////////////////////////////////////////////////
    ...
    virtual void motionImmediateMode( T x, T y, T dx, T dy,  vgModifiers mod) {
        ...
        delta = tVec2(dx,-dy);
        pos   = tVec2( x,  y);
		if      (xRotationModifier & mod) { rotationVector = tVec3::make(T(1), T(0), T(0)); }
        else if (yRotationModifier & mod) { rotationVector = tVec3::make(T(0), T(1), T(0)); }
        else if (zRotationModifier & mod) { rotationVector = tVec3::make(T(0), T(0), T(1)); }
        update();
    }
...
protected:
	...
	tVec3 rotationVector;

    tQuat qtV    ;
    tQuat qtStep ;
    tQuat qtIdle ;

    tVec3 rotationCenter;

    //  settings for the sensitivity
    //////////////////////////////////////////////////////////////////
    T tbScale;    //base scale sensibility
    T fpsRatio;   //auto adjust by FPS (call idle with current FPS)
    T qIdleSpeedRatio; //autoRotation factor to speedup/slowdown
...
//////////////////////////////////////////////////////////////////////
    ...
    static const T constDollyScale = T(.01);
    static const T constPanScale   = T(.01);  //pan scale
    static const T constWheelScale = T(7.5);  //dolly multiply for wheel step
    static const T constDistScale  = T( .1);  //speed by distance sensibility
...
public:
	...
	{
		pan   = tVec3::make(T(0));
		dolly = tVec3::make(T(0));

		dollyScale = constDollyScale;  //dolly scale
		panScale   = constPanScale  ;  //pan scale
		wheelScale = constWheelScale;  //dolly multiply for wheel step
		distScale  = constDistScale ;  //speed by distance sensibility

		povPanDollyFactor = T(0); // internal use, maintain memory of current distance (pan/zoom speed by distance)
	}
    ...
    //////////////////////////////////////////////////////////////////
	tVec3 getPosition() const { return tVec3::make(pan.x, pan.y, dolly.z); }
...
private:
    ...
    T dollyScale ;  //dolly scale
    T panScale   ;  //pan scale
    T wheelScale ;  //dolly multiply for wheel step
    T distScale  ;  //speed by distance sensibility

    T povPanDollyFactor; // internal use, maintain memory of current distance (pan/zoom speed by distance)
};
    ...
    #ifndef IMGUIZMO_USE_ONLY_ROT
        typedef virtualGizmo3DClass vImGuIZMO;
    #else
        typedef virtualGizmoClass vImGuIZMO;
    #endif
    typedef virtualGizmoClass vGizmo;
    typedef virtualGizmo3DClass vGizmo3D;
