...
void MyGameState_GameOptionsMenu::onWillBecomeInactive()
{
	if(settings_did_changed)
	{
		...
		SgGameApp::Get().SaveUserDefinedInGameSettingsToFile();
	}
}

void MyGameState_GameOptionsMenu::Update(
	...
	)
{
	...
	SgUserSettings& user_settings = game.user_settings;
		...
		))
	{
		...
		ImGui::BeginChild(mxHASH_STR("VIDEO"), ImVec2(0, submenu_height), true);
		{
			...
			settings_did_changed |= ImGui::Checkbox(
				"Enable VSync",
				&user_settings.ingame.video.enable_vsync
				);
			if(ImGui::IsItemHovered())
			{
				ImGui::SetTooltip("Synchronizes the game's frame rate with the monitor's refresh rate.");
			}
		}
		ImGui::EndChild();
