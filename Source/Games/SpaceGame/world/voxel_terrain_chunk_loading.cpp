...
void SgWorld::VX_BakeCollisionMesh_AnyThread(
...
void SgWorld::VX_BakeChunkData_AnyThread(
...
void* SgWorld::VX_LoadChunkData_MainThread(
...
void SgWorld::VX_UnloadChunkData_MainThread(
	void* mesh
...
ERet SgWorld::VX_BakeCollisionMesh_AnyThread_Internal(
...
ERet SgWorld::VX_BakeChunkData_AnyThread_Internal(
									  const VX5::MeshBakerI::BakeMeshInputs& inputs
						...
						)
{
	...
	if( use_32_bit_index_buffer ) {
		new_mesh->flags |= MESH_USES_32BIT_IB;
	}
	...
	else
	{
		...
		for( UINT iSubmesh = 0; iSubmesh < submeshes._count; iSubmesh++ )
		{
			...
			//
			Rendering::TbSubmesh	new_submesh;
			{
...
ERet SgWorld::VX_LoadChunkData_MainThread_Internal(
	void *&mesh_
	...
	)
{
	...
	//(VoxelTerrainChunk*) voxels._voxel_terrain_renderer._chunkPool.AllocateItem();
...
new_voxel_terrain_chunk->mesh_inst = nil;
	...
	new_voxel_terrain_chunk->dbg_mesh_offset = CV3f(0);

	new_voxel_terrain_chunk->render_entity_handle = spatial_database->AddEntity(
		new_voxel_terrain_chunk,
...
void SgWorld::VX_UnloadChunkData_MainThread_Internal(
	void* mesh
...
//	spatial_database->OnRegionChanged(chunk_aabb_with_margin_WS);
	...
	{
		spatial_database->RemoveEntity( terrain_chunk->render_entity_handle );
...
void SgWorld::VX_OnLevelLoadProgress(
	const float01_t progress01
	)
{
	SgGameApp::Get().current_loading_progress = progress01;
}

void SgWorld::VX_OnLevelLoaded()
{
	SgGameApp::Get().CompleteLevelLoading();
}
