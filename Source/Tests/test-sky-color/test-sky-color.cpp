...
const char* PATH_TO_SETTINGS = "TestApp_SkyColor.AppSettings.son";
...
mxDEFINE_CLASS(TestApp_SkyColor::AppSettings);
mxBEGIN_REFLECTION(TestApp_SkyColor::AppSettings)
	mxMEMBER_FIELD(atmosphere),
mxEND_REFLECTION;

ERet TestApp_SkyColor::Initialize( const EngineSettings& _settings )
{
	{
...
void TestApp_SkyColor::Shutdown()
{
...
void TestApp_SkyColor::Tick( const InputState& inputState )
{
	DemoApp::Tick(inputState);
	camera.Update(inputState.deltaSeconds);
}
ERet TestApp_SkyColor::Draw( const InputState& inputState )
{
...

	{
		...
		TbShader* shader;
		mxDO(Resources::Load(shader, MakeAssetID("test_sky_color"), &m_sceneData));
		{
			...
			//__declspec(align(16))
			struct AtmosphereParameters_HLSL {
				...
				V3f		f3RayleighSctr;		//!< Rayleigh scattering coefficient, default = ( 5.5e-6, 13.0e-6, 22.4e-6 )
				float	fRayleighHeight;	//!< Rayleigh scale height in meters, default = 8e3

				V3f		f3MieSctr;			//!< Mie scattering coefficient, default = 21e-6
			...
			AtmosphereParameters_HLSL	tmp;
			tmp.f3SunDirection	= m_settings.atmosphere.f3SunDirection;
			tmp.fSunIntensity	= m_settings.atmosphere.fSunIntensity;
			tmp.fInnerRadius	= m_settings.atmosphere.fInnerRadius;
			tmp.fOuterRadius	= m_settings.atmosphere.fOuterRadius;
			tmp.f3RayleighSctr	= m_settings.atmosphere.f3RayleighSctr;
			tmp.fRayleighHeight = m_settings.atmosphere.fRayleighHeight;
			tmp.f3MieSctr		= m_settings.atmosphere.f3MieSctr;
			tmp.fMieHeight		= m_settings.atmosphere.fMieHeight;
			tmp.fMie			= m_settings.atmosphere.fMie;
			mxDO(FxSlow_SetUniform(shader,"u_atmosphere",&tmp));
		}
