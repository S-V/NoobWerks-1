...
#include <Rendering/Public/Globals.h>	// Rendering::RrDirectionalLight
#include <Rendering/Private/Modules/Atmosphere/Atmosphere.h>

//#include <Engine/DevTools.h>
...
#include <VoxelsSupport/VX_DebugHUD_ImGui.h>	// VX5::WorldDebugSettings
...
{
	bool	destructible_environment;

	bool	destructible_floor;

	/// by default is 'false' to avoid NPCs getting stuck in craters, etc.
	bool	shallow_craters;
...
struct MyGameUserSettings: CStruct
{
	InGameSettings		ingame;

	// overrides the engine's default window settings
	NwWindowSettings	window;

public:
	mxDECLARE_CLASS(MyGameUserSettings, CStruct);
	mxDECLARE_REFLECTION;
	MyGameUserSettings();
...
struct GameCheatsSettings: CStruct
{
...
public:
	mxDECLARE_CLASS(GameCheatsSettings, CStruct);
	mxDECLARE_REFLECTION;
	GameCheatsSettings();
};
...
struct MyAppSettings: CStruct
{
	...
	//Engine settings
	bool	dev_show_engine_settings;


public:	//=== Rendering
	// Sky
	Rendering::RrDirectionalLight	sun_light;
	bool							draw_skybox;

public:	//=== VXGI
	bool	dbg_show_bvh;
	int		dbg_max_bvh_depth_to_show;

public:	//=== Voxel Terrain settings:

	bool 	dev_show_voxel_engine_debug_menu;//!< debugging
	VX5::RunTimeEngineSettings		voxel_engine_settings;
	VXExt::WorldDebugSettingsExt	voxel_world_debug_settings;
...
public:	// Debugging / Terrain
	...
	/// Center the clipmap around the fake camera?
	bool	dbg_use_3rd_person_camera_for_terrain_RoI;
	...
	int	window_offset_x;
	int	window_offset_y;
	Rendering::RrGlobalSettings	renderer;
	

	//NEngine::DevTools::RuntimeSettings	engine_settings;
	...
	//
	bool	dbg_disable_parallel_voxels;
...
public:
	mxDECLARE_CLASS(MyAppSettings, CStruct);
	mxDECLARE_REFLECTION;
	MyAppSettings();
