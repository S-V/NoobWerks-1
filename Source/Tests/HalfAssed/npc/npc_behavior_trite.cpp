...
enum
{
	TRITE_HEALTH = 2 * WeaponStats::PLASMAGUN_DAMAGE,

	TRITE_MELEE_ATTACK_DAMAGE = 8,
};
...
///
struct NPC_Behavior_Trite
	: NPC_BehaviorI
	, TInterface<IID_IRCCPP_MAIN_LOOP,IObject>
	, TSingleInstance<NPC_Behavior_Trite>
{
	typedef AI_Brain_Generic_Melee_NPC	AI_Brain_Trite_t;
...
public:
		...
		) override
	{
		...
		//
		npc.comp_health._health = TRITE_HEALTH;

		//
		AI_Brain_Trite_t *	new_ai_data;
		...
		) override
	{
		...
		if(AI_Brain_Trite_t* ai_brain = (AI_Brain_Trite_t*)npc.ai_brain._ptr)
		{
		...
		) override
	{
		...
		//
		AI_Brain_Trite_t& ai_brain = (AI_Brain_Trite_t&) *npc.ai_brain;
		...
		) override
	{
		...
		else
		{
			...
			//
			nwTWEAKABLE_CONST( int, TRITE_GIB_ITEMS_TO_SPAWN, 4 );
			nwTWEAKABLE_CONST( float, TRITE_GIB_ITEM_VELOCITY, 4 );
			...
			game.world.gibs.SpawnGibs(
				...
				, 0.2f // scale
				, TRITE_GIB_ITEMS_TO_SPAWN
				, TRITE_GIB_ITEM_VELOCITY
				);
	...
	{
		return TRITE_MELEE_ATTACK_DAMAGE;
	}
};

REGISTERSINGLETON(NPC_Behavior_Trite,true);
