...
//
int ViewFrustum::PointInFrustum( const Vec3D& point ) const
{
#if 0 
...
//	NOTE: This test assumes frustum planes face inward.
//
int ViewFrustum::IntersectSphere( const Sphere& sphere ) const
{
...
//	NOTE: This test assumes frustum planes face inward.
//
int ViewFrustum::IntersectsAABB( const AABB& aabb ) const
{
...
void ViewFrustum::CalculateCornerPoints( Vec3D corners[8] ) const
{
	for( UINT iPoint = 0; iPoint < 8; iPoint++ )  
	{
		...
		const bool bOk = PlanesIntersection( p0, p1, p2, corners[iPoint] );
		mxASSERT(bOk);
		(void)bOk;
	}
}
...
//
int ViewFrustum::CullPoints( const Vec3D* points, UINT numPoints ) const
{
	mxOPTIMIZE(:);
