...
void Redirect_IO_To_Console()
{
	...
	if( !bOk ) {
		ptWARN("Failed to Initialize debug console.");
	}
...
CConsole::CConsole(const char* szTitle)
{
	...
	if( !Create(szTitle) )
	{
		ptWARN("Failed to create console window with title %s",szTitle);
	}
}
...
void CConsole::Put( const TCHAR* msg )
{
	mxASSERT_PTR(msg);
	DWORD	dwWritten;
	// if not parameter set, write a new line
	WriteConsole( hConsole, msg, _tcsclen( msg ), &dwWritten, 0 );
}
...
mxSTOLEN("https://github.com/skandhas/color-print/blob/master/color_print.c");
/* Colors */
enum  EConsoleColor
{
  CP_BLACK        = 0,
...
WinConsoleLog::WinConsoleLog()
{
	m_default_color = CP_WHITE;
	m_current_color = CP_WHITE;
	m_std_output = INVALID_HANDLE_VALUE;
}

ERet WinConsoleLog::Initialize()
{
	// Get handle to standard output
	m_std_output = ::GetStdHandle(STD_OUTPUT_HANDLE);
	mxENSURE(m_std_output != INVALID_HANDLE_VALUE, ERR_UNKNOWN_ERROR, "Failed to get STD_OUTPUT_HANDLE");

	CONSOLE_SCREEN_BUFFER_INFO coninfo;
	mxENSURE( ::GetConsoleScreenBufferInfo( m_std_output, &coninfo ) == TRUE, ERR_UNKNOWN_ERROR, "GetConsoleScreenBufferInfo() failed");

	m_default_color = coninfo.wAttributes;
	m_current_color = m_default_color;

	return ALL_OK;
}

void WinConsoleLog::VWrite( ELogLevel _level, const char* _message, int _length )
{
	int	new_color = m_current_color;
	switch( _level )
	{
	case LL_Trace :	new_color = CP_DARK_GREEN;	break;
	case LL_Debug :	new_color = CP_DARK_GRAY;	break;
	case LL_Info :	new_color = m_default_color;	break;
	case LL_Warn :	new_color = CP_YELLOW;	break;
	case LL_Error :	new_color = CP_RED;	break;
	case LL_Fatal :	new_color = CP_MAGENTA;	break;
	default: /* nothing */;
	}

	if( m_current_color != new_color )
	{
		CHK( ::SetConsoleTextAttribute( m_std_output, new_color ) == TRUE );
