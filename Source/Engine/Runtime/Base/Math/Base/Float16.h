...
public:
	mxFORCEINLINE mxFloat16()
	{}

	mxFORCEINLINE mxFloat16( const FLOAT32& f )
	{
		INT32 t = ((const INT32*)&f)[0];
		value = INT16( t >> 16 );
	}

	mxFORCEINLINE mxFloat16 & operator = ( const FLOAT32& f )
	{
		INT32 t = ((const INT32*)&f)[0];
		value = INT16( t >> 16 );
		return *this;
	...
	mxFORCEINLINE operator FLOAT32() const
	{
		union
		{
			INT32 i;
...
//
mxFORCEINLINE BYTE FloatToByte( float x )
{
	union {
		FLOAT f;
		UINT  i;
