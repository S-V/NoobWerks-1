...
mxFORCEINLINE bool IsSpecial( const FLOAT& f )
{
	_flint u;
	u.f = f;
	DWORD bits = u.i;
...
// Don't use these macros, use unions instead.
mxFORCEINLINE bool IsNAN( FLOAT f )				{ return (UIR(f) & 0x7F800000) == 0x7F800000;	}
mxFORCEINLINE bool IsIndeterminate( FLOAT f)	{ return (UIR(f) == 0xFFC00000);				}
mxFORCEINLINE bool IsPlusInf( FLOAT f )			{ return (UIR(f) == 0x7F800000);				}
mxFORCEINLINE bool IsMinusInf( FLOAT f )		{ return (UIR(f) == 0xFF800000);				}

mxFORCEINLINE bool IsValidFloat( FLOAT f )
{
...
/*
mxFORCEINLINE bool FloatIsNan( FLOAT x )
{
	return x != x;
}
*/
...
mxFORCEINLINE FLOAT mxComputeFloatEpsilon()
{
	FLOAT f = 1.0f;
	((ULONG&)f) ^= 1;
	return f - 1.0f;	// it's the same as FLT_EPSILON
}

mxFORCEINLINE bool mxFuzzyIsZero( FLOAT x, FLOAT epsilon = 1e-6f )
{
	return (x * x) < epsilon;
}

mxFORCEINLINE FLOAT minf( FLOAT a, FLOAT b )
{
	return (a < b) ? a : b;
}

mxFORCEINLINE FLOAT maxf( FLOAT a, FLOAT b )
{
	return (a > b) ? a : b;
}

mxFORCEINLINE FLOAT midf( FLOAT a, FLOAT b )
{
	return (a + b) * 0.5f;
}

mxFORCEINLINE FLOAT clampf( FLOAT x, FLOAT min, FLOAT max )
{
	return (x < min) ? min : (x > max) ? max : x;
}

mxFORCEINLINE FLOAT fselect( FLOAT test, FLOAT a, FLOAT b )
{
	return (test >= 0.0f ) ? a : b;	// branchless selection (using conditional move)
}

mxFORCEINLINE void swapf( FLOAT & x, FLOAT & y )
{
	FLOAT temp = x;
	x = y;
	y = temp;
...
mxFORCEINLINE FLOAT squaref( FLOAT x )
{
	return (x * x);
}

mxFORCEINLINE FLOAT cubef( FLOAT x )
{
	return (x * x * x);
}

mxFORCEINLINE FLOAT FourthPower( FLOAT x )
{
	FLOAT sq = x * x;
	return sq * sq;
}

mxFORCEINLINE FLOAT signf( FLOAT x )
{
	return (x > 0.0f) ? 1.0f
		: (x < 0.0f) ? -1.0f
		: 0.0f;
...
mxFORCEINLINE BOOL mxDifferentSign( FLOAT x, FLOAT y )
{
	return (UIR(x)^UIR(y)) & F32_SIGN_BITMASK;
}

mxFORCEINLINE bool mxFuzzyIsNull( FLOAT x, FLOAT epsilon = 1e-5 )
{
	return (fabs( x ) <= epsilon);
}

// Tests if the two values are equal (with respect to specified epsilon).
mxFORCEINLINE bool mxFuzzyEqual( FLOAT x, FLOAT y, FLOAT epsilon = 1e-5 )
{
...
mxFORCEINLINE INT mxFtoI( FLOAT x )
{
...
mxFORCEINLINE INT QuickFist( float inval )
{
...
/// Yossarian King, "Floating-Point Tricks: Improving Performance with IEEE Floating Point",
/// in Game Programming Gems 2 , ed. M. DeLoura, Charles River Media, Hingham, MA., pp. 167-181, 2001.
mxFORCEINLINE INT FloatToInt( FLOAT x )
{
	_flint	result, bias;
	bias.i = (x > 0.0f)
		?
...
// Slow!
mxFORCEINLINE INT mxFloatToInt( FLOAT x )
{
	return mxFtoI( x );
}

// Slow!
mxFORCEINLINE FLOAT mxIntToFloat( INT x )
{
	return (FLOAT) x;
}
...
{
	return scale * (FLOAT)floor( (x / scale) + 0.5f );
}
...
*/
mxFORCEINLINE INT mxDoubleToInt( DOUBLE val )
{
...
//
mxINLINE INT mxScaledFloatToInt( FLOAT value, INT power )
{
	INT result;
...
mxINLINE bool NumberValid(const float& x)
{
...
mxINLINE bool NumberNAN(const float& x)
{
	return (((FloatU32(x) & FloatU32ExpMask) == FloatU32ExpMask) && 
					((FloatU32(x) & FloatU32FracMask) != 0));
}

mxINLINE bool NumberINF(const float& x)
{
	return (((FloatU32(x) & FloatU32ExpMask) == FloatU32ExpMask) && 
					((FloatU32(x) & FloatU32FracMask) == 0));
}

mxINLINE bool NumberDEN(const float& x)
{
...
mxINLINE bool NumberValid(const double& x)
{
	return ((DoubleU64(x) & DoubleU64ExpMask) != DoubleU64ExpMask);
}

mxINLINE bool NumberNAN(const double& x)
{
	return (((DoubleU64(x) & DoubleU64ExpMask) == DoubleU64ExpMask) && 
					((DoubleU64(x) & DoubleU64FracMask) != 0));
}

mxINLINE bool NumberINF(const double& x)
{
	return (((DoubleU64(x) & DoubleU64ExpMask) == DoubleU64ExpMask) && 
					((DoubleU64(x) & DoubleU64FracMask) == 0));
}

mxINLINE bool NumberDEN(const double& x)
{
...
mxINLINE bool NumberValid(const INT8 x)
{	
	return true; //integers are always valid
}
mxINLINE bool NumberValid(const UINT8 x)
{	
	return true; //integers are always valid
}
mxINLINE bool NumberValid(const INT16 x)
{	
	return true; //integers are always valid
}
mxINLINE bool NumberValid(const UINT16 x)
{	
	return true; //integers are always valid
}
mxINLINE bool NumberValid(const INT32 x)
{	
	return true; //integers are always valid
}
mxINLINE bool NumberValid(const UINT32 x)
{	
	return true; //integers are always valid
}
mxINLINE bool NumberValid(const INT64 x)
{	
	return true; //integers are always valid
}
mxINLINE bool NumberValid(const UINT64 x)
{	
	return true; //integers are always valid
}
...
//	RaiseBy2toN
//
mxFORCEINLINE FLOAT RaiseBy2toN( FLOAT x, INT n )
{
...
//	ClampChar
//
mxINLINE signed char ClampChar( int i )
{
	if ( i < -128 ) {
		return -128;
	}
...
//	ClampShort
//
mxINLINE signed short ClampShort( int i )
{
	if ( i < -32768 ) {
		return -32768;
	}
...
//	WrapInt
//
mxINLINE int WrapInt( int x, int hi )
{
	if( x < 0 ) {
		return ( (x % hi) + hi ) % hi;
	} else {
...
mxINLINE int GetPow2( unsigned long u )
{
	if ( u > 1 && (u & (u - 1)) == 0 )
	{
		for ( int n = 0; u; u >>= 1, n++ )
...
mxINLINE TYPE Factorial( TYPE x )
{
	TYPE result = 1;
	while( x > 0 )
	{
...
mxINLINE UINT IntFactorial( UINT x )
{
...
FLOAT TetrahedronVolume(
			...
			const V3f& v2, const V3f& v3 )
{
	return fabs( TetrahedronDeterminant(v0,v1,v2,v3) * 0.16666666666666666667f );	// * (1.0f / 6.0f)
}
#endif
...
static float CosineStep(float t)
{
	return 0.5f - 0.5f * cos(mxPI * t);
}
...
static float PowerCurve( float x, float a, float b )
{
...
/// denotes a float that must be in range [0..1]
typedef	float	float01_t;

/// denotes a time interval measured in seconds
typedef float SecondsF;
typedef float SecondsD;
