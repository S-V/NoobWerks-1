...
mxSTOLEN("Valve Source Engine (L4D 2 SDK)");
...
template<class T> inline void AssertValidReadPtr( T* ptr, int count = 1 )		     { mxASSERT_PTR( (void*)ptr ); }
template<class T> inline void AssertValidWritePtr( T* ptr, int count = 1 )		     { mxASSERT_PTR( (void*)ptr ); }
template<class T> inline void AssertValidReadWritePtr( T* ptr, int count = 1 )	     { mxASSERT_PTR( (void*)ptr ); }
...
int	_V_strlen(const char* file, int line, const char *_str)
{
	AssertValidStringPtr(_str);
	return strlen( _str );
}
...
const char *StringAfterPrefix( const char *_str, const char *prefix )
{
	AssertValidStringPtr( _str );
	...
	while ( tolower( *_str++ ) == tolower( *prefix++ ) );
	return NULL;
}

const char *StringAfterPrefixCaseSensitive( const char *_str, const char *prefix )
{
	AssertValidStringPtr( _str );
	AssertValidStringPtr( prefix );
	do
...
{
	mxASSERT( path );
	mxASSERT( pathStringLength >= 1 );
	mxASSERT( extension );
	mxASSERT( extension[0] == '.' );
...
void V_MakeAbsolutePath( char *pOut, int outLen, const char *pPath, const char *pStartingDir )
{
	...
	else
	{
		...
		else
		{
			if ( !_getcwd( pOut, outLen ) )
				ptERROR( "V_MakeAbsolutePath: _getcwd failed." );
	...
	if ( !V_RemoveDotSlashes( pOut ) )
		ptERROR( "V_MakeAbsolutePath: tried to \"..\" past the root." );
...
// https://stackoverflow.com/a/7718223/4232223
