...
#define V_strlen(_str)					_V_strlen	(__FILE__, __LINE__, (_str))				
...
inline int		V_strlen (const char *_str)							{ return (int) strlen ( _str ); }
...
// returns string immediately following prefix, (ie _str+strlen(prefix)) or NULL if prefix not found
...
inline bool	StringHasPrefix             ( const char *_str, const char *prefix ) { return StringAfterPrefix             ( _str, prefix ) != NULL; }
inline bool	StringHasPrefixCaseSensitive( const char *_str, const char *prefix ) { return StringAfterPrefixCaseSensitive( _str, prefix ) != NULL; }
...
// get a pointer to the file extension with dot (e.g. "txt")
