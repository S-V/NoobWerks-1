	...
	)
{
	...
	//const float3 origin_VS = mul( g_view_matrix, float4( particle_origin_WS, 1 ) ).xyz;
	...
	//const float3 local_right	= float3( radius,		0	  ,		0   );
	//const float3 local_up		= float3( 0     ,		radius,		0	);
	//const float3 local_right	= g_inverse_view_matrix[0].xyz * radius;
	//const float3 local_up		= g_inverse_view_matrix[2].xyz * radius;

	//const float3 local_up		= float3(0,0,1) * radius;
	//const float3 local_right	= -normalize(cross( eye_to_particle_dir, local_up )) * radius;
