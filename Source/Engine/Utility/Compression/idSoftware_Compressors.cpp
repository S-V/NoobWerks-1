#include <Base/Base.h>
#pragma hdrstop
#include <Utility/Compression/idSoftware_Compressors.h>
...
int idCompressor_None::Seek( long offset, fsOrigin_t origin ) {
	ptERROR( "cannot seek on idCompressor" );
	return -1;
}
...
void idCompressor_BitStream::Init( idFile *f, bool compress, int wordLength ) {
	...
	mxASSERT( wordLength >= 1 && wordLength <= 32 );
...
================
*/
mxINLINE void idCompressor_BitStream::InitCompress( const void *inData, const int inLength ) {
...
================
*/
mxINLINE void idCompressor_BitStream::InitDecompress( void *outData, int outLength ) {
